...
def excel_to_pdf(excel_file, output_pdf):
    ...
    try:
        excel = win32com.client.DispatchEx("Excel.Application")  # DispatchEx 创建独立进程，不复用已打开的 Excel
        excel.Visible = False         # 必须在 Open 之前设置，避免窗口闪现
        excel.DisplayAlerts = False
        excel.ScreenUpdating = False  # 关闭屏幕刷新，加快速度
        excel.Interactive = False     # 禁止用户交互，防止弹窗打断

        wb = excel.Workbooks.Open(os.path.abspath(excel_file), ReadOnly=True)
        ws = wb.Worksheets(1)
...
def main():
    ...
    if '4' in choices and survey_files_4:
        ...
        for f in survey_files_4:
            ...
            if page_num:
                print(f"   ✅ {os.path.basename(f)} - 地籍调查成果验收报告在第 {page_num} 页")
                acceptance_tasks.append((f, page_num))
            else:
                print(f"   ❌ {os.path.basename(f)} - 未找到'地籍调查成果验收报告'")
